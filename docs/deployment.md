...
This document describes how to do a full deployment of SND on your own Linux server. You will need:
...
* First, remove that htdocs folder - we will replace it with SND's version now
* git clone https://github.com/tarnover/snd.git htdocs
...
To have a permanently running version of SND as a background process:
...
Now the SND backend should be running on port 1443. You can check with:
...
Of course, we don't want to expose the service on port 1443. Instead we want our normal webserver to forward all requests to SND ("Reverse proxy").
...
# Make sure the original domain name is forwarded to SND
...
# If it's a websocket connection, redirect it to a SND WS connection
